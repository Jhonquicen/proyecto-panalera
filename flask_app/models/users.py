...
class User:
    ...
    def save(cls, formulario):
        ...
        result = connectToMySQL('proyecto_pañalera').query_db(query, formulario) #me regresan el nuevo ID de la persona registrada
        #result = 5
        return result
    ...
    def valida_usuario(formulario):
        ...
        results = connectToMySQL('proyecto_pañalera').query_db(query, formulario)
        if len(results) >= 1:
    ...
    def get_by_email(cls, formulario):
        ...
        #}
        query = "SELECT * FROM users WHERE email = %(email)s"
        result = connectToMySQL('proyecto_pañalera').query_db(query, formulario) #Los SELECT regresan una lista
